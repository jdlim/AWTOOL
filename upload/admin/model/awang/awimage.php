...
function error_handler_for_awimage($errno, $errstr, $errfile, $errline) {
	global $registry;
	...
	if (($errors != "Fatal Error") && isset($request->get['route']) && ($request->get['route']!='awang/awimage'))  {
		...
		$link = $url->link( 'awang/awimage', 'token='.$token, 'SSL' );
		header('Status: ' . 302);
...
function fatal_error_shutdown_handler_for_awimage()
{
	$last_error = error_get_last();
	if ($last_error['type'] === E_ERROR) {
		// fatal error
		error_handler_for_awimage(E_ERROR, $last_error['message'], $last_error['file'], $last_error['line']);
	}
}


class ModelAwangAwimage extends Model {
...
function storeProductImage() {
	...
	$fullpath = 'C:/wamp/www/store/image/';
	$path = realpath($fullpath.'catalog/products');
	$this->clearCache();
	$database =& $this->db;
	// start transaction, remove product images from database
	...
	try {
		...
		$result = array();
		$query = "SELECT product_id, model FROM oc_product;";
		$result = $database->query($query);
		// TODO: Add the hash table code here
//		var_dump($result);
		$img_lookup = array();
		foreach ($result as $element) {
			if (is_array($element)){
			foreach ($element as $brow) {
				if (isset($brow["model"])) {
//			echo $brow["model"]." is model and ".$brow["product_id"]." is product_id <br>";
		    $img_lookup[$brow["model"]] = $brow["product_id"];
			}}
		}}
//			print_r($img_lookup);
	$ite=new RecursiveDirectoryIterator($path);
	$iterat = new RecursiveIteratorIterator($ite);
	$nbfiles=0;
			foreach ($img_lookup as $modelname => $productid) {
				$nbfiles++;
				 $imageurl = "http:\\\\www.ronlynn.com\\images\\wang\\" . $modelname .".jpg";
				 $imageurl = addslashes($imageurl);
	 			$sql2 = "INSERT INTO `".DB_PREFIX."product_image` (`product_image_id`,`product_id`,`image`,`sort_order`) VALUES ";
	 			$sql2 .= "('0','" .$productid ."', \"". $imageurl ."\",'0');";
				$database->query( $sql2 ); 
			}
	
	    foreach ($iterat as $filename=>$cur) {
			if (!$cur->isDir()) {
				 $filefoo=$cur->getFilename();
				 $filename=realpath($filename);
				 $filename = str_replace('\\', '/', $filename);
				 $fullpath = str_replace('\\', '/', $fullpath);
				 $filebar = str_replace($fullpath, '', $filename);
//				echo "$fullpath => $filename => $filebar <br>";
				$arr = explode('_', trim($filefoo));
				if ($arr[0]===$filefoo) {
					$arr = explode(' ', trim($filefoo));
				}
				// TODO: Lookup code here & store to database here
				$modelname = $arr[0];
//				if (isset($img_lookup[$modelname])) {
//				echo "Found something for [$modelname] <br>";
//		     $nbfiles++;
//				}
				if (isset($img_lookup[$modelname])) {
			     $nbfiles++;
	 			$sql2 = "INSERT INTO `".DB_PREFIX."product_image` (`product_image_id`,`product_id`,`image`,`sort_order`) VALUES ";
	 			$sql2 .= "('0',$img_lookup[$modelname], '".$database->escape($filebar)."','0');";
				$database->query( $sql2 ); 
			 	}  
/*				
				else {
			     $nbfiles++;
				 $imageurl = "http:\\\\www.ronlynn.com\\images\\wang\\" . $modelname;
	 			$sql2 = "INSERT INTO `".DB_PREFIX."product_image` (`product_image_id`,`product_id`,`image`,`sort_order`) VALUES ";
	 			$sql2 .= "('0'," .$modelname .", ". $imageurl .",'0');";
				$database->query( $sql2 ); 					
				}
				  */
		 	}
		...
		$this->session->data['img_success'] = "SUCCESS: $nbfiles images was added to the catalog";
		$this->session->data['log'] = "$nbfiles images were added to the database"."\n";
//		echo "$nbfiles images were added to the database! <br>";
//		die();
		return TRUE;
	 ...
	 // TODO: Do we delete before adding again?		
	}
}
...
function checkme() {
		...
		set_error_handler('error_handler_for_awimage',E_ALL);
		register_shutdown_function('fatal_error_shutdown_handler_for_awimage');

		try {
			$database =& $this->db;
			$this->session->data['export_nochange'] = 1;
			// SELECT oc_product.product_id, oc_product.model FROM oc_product LEFT JOIN oc_product_image ON oc_product.product_id = oc_product_image.product_id WHERE oc_product_image.product_id IS NULL;
			$result = array();
			$query = "SELECT oc_product.product_id, oc_product.model FROM oc_product LEFT JOIN oc_product_image ON oc_product.product_id = oc_product_image.product_id WHERE oc_product_image.product_id IS NULL;";
			$result = $database->query($query);
//			$this->session->data['log'] .= print_r($result);
				if ($result->rows) {
					foreach ($result->rows as $row) {
						$this->session->data['log'] .= $row['model']."\n";
					}
				}			
			$this->session->data['log'] .= "\n Num Rows =[".$result->num_rows."]\n";
			// TODO: Pass this to the view to be displayed as a list of styles with missing images. Total number should be reported in error box.
			return TRUE;
		} catch (Exception $e) {
			$errstr = $e->getMessage();
			$errline = $e->getLine();
...
// TODO:	Need to check into Manufactures to Store ID. Skipping this SQL for now.
	...
	function storeProductsIntoDatabase( &$database, &$products ) 
	{
		...
		foreach ($products as $product) {
			...
			$sql .= "quantity=VALUES(quantity), sku=VALUES(sku), upc=VALUES(upc), ean=VALUES(ean), jan=VALUES(jan), isbn=VALUES(isbn), mpn=VALUES(mpn), location=VALUES(location), ";
			$sql .= "stock_status_id=VALUES(stock_status_id), model=VALUES(model), manufacturer_id=VALUES(manufacturer_id), image=VALUES(image), shipping=VALUES(shipping), price=VALUES(price), points=VALUES(points), date_added=VALUES(date_added), date_modified=VALUES(date_modified), date_available=VALUES(date_available), weight=VALUES(weight), weight_class_id=VALUES(weight_class_id), status=VALUES(status), ";
			$sql .= "tax_class_id=VALUES(tax_class_id), viewed=VALUES(viewed), length=VALUES(length), width=VALUES(width), height=VALUES(height), length_class_id=VALUES(length_class_id), sort_order=VALUES(sort_order), subtract=VALUES(subtract), minimum=VALUES(minimum);";
			$database->query($sql);
//			echo $sql."<br>";
...
//			echo "[$lastone] = product $productName <br>";
			...
			$sql2 .= "language_id=VALUES(language_id), name=VALUES(name), description=VALUES(description), meta_description=VALUES(meta_description), meta_keyword=VALUES(meta_keyword), tag=VALUES(tag);";
...
//if ($topcategory==$cat_lookup['15']) {echo "TRUE <br>";} else {echo "FALSE <br>";}
			...
			$sku = $this->getCell($data,$i,$cnscol['COLOR'],'');
			...
			$manufacturer = $this->getCell($data,$i,$cnscol['SEA']);
			$imageName = '';
			$shipping = 'yes';
...
//			}
			...
			$description = " <p><b>STYLE NAME: </b>" . $model . "</p><b>COLOR: </b>" . $style_color_name . "</p><b> GROUP: </b>" . $category_low;
...
//		die( "Inside of uploadATSproduc..[".count($products)."] was loaded..<br>");
...
//		$result = mysqli_query($dbc, $query);
		...
		return storeCategoriesIntoDatabase( $database, $categories );
	}
	...
	protected function expectedRLMProductsHeading() {
		return array( "Whse #", "Div #", "Division Name", "Sea", "Style", "Style Description", "Fabric", "", "Color", "Color Name", "F&W", "Style Status", "Size Scale", "Size", "UPC", "Group", "Category", "Sub-Div", "WS Price", "Unit Cost", "Cost Type", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost", "Units", "WS Price", "Cost");
	}
